...
#define STICK_OVERRIDE_DEADBAND        500  
#define STICK_OVERRIDE_LOITER_SPEED_MS 5.0f 
...
bool ModeCustom::takeoff_complete;      // true once takeoff has completed (used to trigger retracting of landing gear)


bool ModeCustom::stick_override_active() const
{
    ...
    // rc().channel() returns raw pwm (typically 1000-2000), midpoint = 1500
    // We compare deviation from centre against the deadband
    const int16_t roll_in   = copter.channel_roll->get_control_in();    // already centred / scaled by AP
    const int16_t pitch_in  = copter.channel_pitch->get_control_in();
    const int16_t yaw_in    = copter.channel_yaw->get_control_in();

    // Throttle: we want to detect intentional movement, so compare against
    // the "hover" midpoint rather than absolute zero

    return (abs(roll_in)  > STICK_OVERRIDE_DEADBAND ||
            abs(pitch_in) > STICK_OVERRIDE_DEADBAND ||
            abs(yaw_in)   > STICK_OVERRIDE_DEADBAND );
}


void ModeCustom::update_stick_override()
{
    // Already latched — nothing more to do.
    if (_stick_override_engaged) {
        return;
    }

    if (stick_override_active()) {
        _stick_override_engaged = true;

        // Initialise loiter-style speed limits
        pos_control->NE_set_max_speed_accel_m(STICK_OVERRIDE_LOITER_SPEED_MS, wp_nav->get_wp_acceleration_mss());
        pos_control->NE_set_correction_speed_accel_m(STICK_OVERRIDE_LOITER_SPEED_MS, wp_nav->get_wp_acceleration_mss());

        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "Custom: Stick override — switch mode to resume guided");
    }
}

void ModeCustom::stick_override_run()
{
    // if not armed / landed, safe-out
    ...
    const float lean_max_rad = attitude_control->get_althold_lean_angle_max_cd() * 0.01f * DEG_TO_RAD;
    const float lean_max_cd  = lean_max_rad * RAD_TO_DEG * 100.0f; 

    // Horizontal: roll/pitch sticks → NE velocity
    float target_roll_rad, target_pitch_rad;
    get_pilot_desired_lean_angles_rad(target_roll_rad, target_pitch_rad,
                                        lean_max_cd,
                                        lean_max_rad);

    // Scale lean angle → horizontal speed (simple proportional mapping)
    float vel_n_ms = 0.0f, vel_e_ms = 0.0f;
    if (!is_zero(lean_max_rad)) {
        vel_n_ms = -target_pitch_rad / lean_max_rad * STICK_OVERRIDE_LOITER_SPEED_MS;
        vel_e_ms =  target_roll_rad  / lean_max_rad * STICK_OVERRIDE_LOITER_SPEED_MS;
    }

    // Rotate from body frame to NED (use vehicle heading)
    const float yaw_rad = ahrs.get_yaw();
    const float cos_yaw = cosf(yaw_rad);
    const float sin_yaw = sinf(yaw_rad);
    Vector2f vel_ne_ms;
    vel_ne_ms.x = vel_n_ms * cos_yaw - vel_e_ms * sin_yaw;
    vel_ne_ms.y = vel_n_ms * sin_yaw + vel_e_ms * cos_yaw;

    // Vertical: throttle stick → climb rate (same as AltHold)
...
bool ModeCustom::init(bool ignore_checks)
{
    ...
    _stick_override_engaged  = false;
    // _sticks_centered_time_ms = 0;
...
void ModeCustom::run()
{
    ...
    if (!_paused && guided_mode != SubMode::TakeOff && guided_mode != SubMode::Angle) {
           update_stick_override();
    }
   
    // If stick override is engaged, run Loiter-style control and return
    if (_stick_override_engaged) {
        stick_override_run();
        return;
    }
...
bool ModeCustom::option_is_enabled(Option option) const
{
...
bool ModeCustom::move_vehicle_on_ekf_reset() const
{
        // call the correct auto controller
    switch (guided_mode) {
    case SubMode::TakeOff:
...
bool ModeCustom::do_user_takeoff_start_m(float takeoff_alt_m)
{
...
bool ModeCustom::set_speed_NE_ms(float speed_ne_ms)
{
    // initialise horizontal speed, acceleration
...
bool ModeCustom::set_speed_up_ms(float speed_up_ms)
{
    // initialize vertical speeds and acceleration
...
bool ModeCustom::set_speed_down_ms(float speed_down_ms)
{
    // initialize vertical speeds and acceleration
...
bool ModeCustom::set_pos_NED_m(const Vector3p& pos_ned_m, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw, bool is_terrain_alt)
{
#if AP_FENCE_ENABLED
...
bool ModeCustom::set_destination(const Location& dest_loc, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw)
{
#if AP_FENCE_ENABLED
...
void ModeCustom::set_accel_NED_mss(const Vector3f& accel_ned_mss, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw, bool log_request)
{
...
void ModeCustom::set_vel_NED_ms(const Vector3f& vel_ned_ms, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw, bool log_request)
{
    set_vel_accel_NED_m(vel_ned_ms, Vector3f(), use_yaw, yaw_rad, use_yaw_rate, yaw_rate_rads, relative_yaw, log_request);
}

// set_vel_accel_NED_m - sets guided mode's target velocity and acceleration
void ModeCustom::set_vel_accel_NED_m(const Vector3f& vel_ned_ms, const Vector3f& accel_ned_mss, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw, bool log_request)
{
    // check we are in velocity and acceleration control mode
    if (guided_mode != SubMode::VelAccel) {
        velaccel_control_start();
...
bool ModeCustom::set_pos_vel_NED_m(const Vector3p& pos_ned_m, const Vector3f& vel_ned_ms, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw)
{
...
bool ModeCustom::set_pos_vel_accel_NED_m(const Vector3p& pos_ned_m, const Vector3f& vel_ned_ms, const Vector3f& accel_ned_mss, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw)
{
#if AP_FENCE_ENABLED
    // reject destination if outside the fence
...
bool ModeCustom::stabilizing_pos_NE() const
{
...
bool ModeCustom::stabilizing_vel_NE() const
{
...
void ModeCustom::set_angle(const Quaternion &attitude_quat, const Vector3f &ang_vel_body, float climb_rate_ms_or_thrust, bool use_thrust)
{
...
void ModeCustom::pos_control_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        // do not spool down tradheli when on the ground with motor interlock enabled
...
void ModeCustom::set_yaw_state_rad(bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_angle)
{
...
void ModeCustom::limit_set(uint32_t timeout_ms, float alt_min_m, float alt_max_m, float horiz_max_m)
{
...
const Vector3p &ModeCustom::get_target_pos_NED_m() const
{
    return guided_pos_target_ned_m;
}

const Vector3f& ModeCustom::get_target_vel_NED_ms() const
{
    return guided_vel_target_ned_ms;
}

const Vector3f& ModeCustom::get_target_accel_NED_mss() const
{
    return guided_accel_target_ned_mss;
}

float ModeCustom::wp_distance_m() const
{
    switch(guided_mode) {
    case SubMode::WP:
...
float ModeCustom::wp_bearing_deg() const
{
    switch(guided_mode) {
    case SubMode::WP:
...
float ModeCustom::crosstrack_error_m() const
{
    switch (guided_mode) {
    case SubMode::WP:
